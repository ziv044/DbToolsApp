# Story 2.2: Server Connection Validation

## Status: Ready for Review

## Story

**As a** user,
**I want** to test connectivity when adding a server,
**so that** I know the connection details are correct before saving.

## Acceptance Criteria

| # | Criteria |
|---|----------|
| 1 | `POST /api/servers/test-connection` tests connectivity without saving |
| 2 | Uses pyodbc to attempt connection to SQL Server |
| 3 | Returns success with SQL Server version and edition on connect |
| 4 | Returns failure with descriptive error (timeout, auth failed, unreachable) |
| 5 | Connection test has 10-second timeout |
| 6 | Validates minimum SQL Server version (2016+) |
| 7 | Checks if connected user has required permissions (VIEW SERVER STATE) |
| 8 | `POST /api/servers` can include `validate: true` to test before saving |
| 9 | If validation fails with `validate: true`, server is not created |

## Tasks / Subtasks

- [x] Task 1: Create SQL Server connector (AC: 2, 5)
  - [x] Create `backend/app/connectors/__init__.py`
  - [x] Create `backend/app/connectors/sqlserver.py`
  - [x] Implement connection string builder
  - [x] Configure 10-second timeout

- [x] Task 2: Implement test_connection method (AC: 3, 4, 6, 7)
  - [x] Execute `SELECT @@VERSION` for version info
  - [x] Check for SQL Server 2016+ (version 13+)
  - [x] Check VIEW SERVER STATE permission
  - [x] Return structured success/error response

- [x] Task 3: Create test-connection endpoint (AC: 1)
  - [x] Add POST /api/servers/test-connection
  - [x] Accept connection parameters in body
  - [x] Return test results

- [x] Task 4: Add validation to create endpoint (AC: 8, 9)
  - [x] Accept `validate: true` in POST body
  - [x] Run connection test if validate=true
  - [x] Return error without saving if test fails

## Dev Notes

### SQL Server Connector
```python
# backend/app/connectors/sqlserver.py
import pyodbc
from flask import current_app

class SQLServerConnector:
    def _build_connection_string(self, hostname, port, instance_name,
                                  auth_type, username=None, password=None):
        driver = current_app.config.get('SQLSERVER_DRIVER', 'ODBC Driver 18 for SQL Server')

        if instance_name:
            server = f"{hostname}\\{instance_name}"
        else:
            server = f"{hostname},{port}"

        conn_str = f"DRIVER={{{driver}}};SERVER={server};DATABASE=master;"

        if auth_type == 'windows':
            conn_str += "Trusted_Connection=yes;"
        else:
            conn_str += f"UID={username};PWD={password};"

        conn_str += "TrustServerCertificate=yes;"
        return conn_str

    def test_connection(self, **kwargs) -> dict:
        try:
            conn_str = self._build_connection_string(**kwargs)
            conn = pyodbc.connect(conn_str, timeout=10)
            cursor = conn.cursor()

            # Get version
            cursor.execute("SELECT @@VERSION")
            version = cursor.fetchone()[0]

            # Check permission
            cursor.execute("SELECT HAS_PERMS_BY_NAME(null, null, 'VIEW SERVER STATE')")
            has_permission = cursor.fetchone()[0] == 1

            conn.close()

            return {
                'success': True,
                'version': version,
                'has_permission': has_permission
            }
        except pyodbc.Error as e:
            return {
                'success': False,
                'error': str(e)
            }
```

### Required Dependencies
```
# requirements.txt
pyodbc>=5.0.0
```

### Error Response Examples
```json
// Success
{"success": true, "version": "Microsoft SQL Server 2019...", "has_permission": true}

// Failure
{"success": false, "error": "Login failed for user 'sa'"}
{"success": false, "error": "Connection timeout expired"}
```

## Testing

### Test Requirements
- Test connection string building
- Test successful connection response
- Test failure scenarios (mock pyodbc)
- Test timeout handling
- Location: `backend/tests/unit/test_sqlserver_connector.py`

## Change Log

| Date | Version | Description | Author |
|------|---------|-------------|--------|
| 2026-01-17 | 0.1 | Initial story creation | PM (John) |

---

## Dev Agent Record

### Agent Model Used
Claude Opus 4.5 (claude-opus-4-5-20251101)

### Debug Log References
- None - implementation proceeded without issues

### Completion Notes List
- SQLServerConnector class with connection string builder
- Supports SQL auth and Windows auth
- Named instances handled correctly (hostname\instance format)
- 10-second connection timeout configured
- test_connection method returns ConnectionTestResult with:
  - SQL Server version (@@VERSION)
  - Edition (Enterprise/Standard/Developer/Express/Web)
  - Major version for 2016+ validation (version 13+)
  - VIEW SERVER STATE permission check
- Error categorization with user-friendly messages (AUTH_FAILED, CONNECTION_FAILED, TIMEOUT, DRIVER_NOT_FOUND, etc.)
- POST /api/servers/test-connection endpoint added
- POST /api/servers accepts `validate: true` parameter
- Graceful handling when pyodbc not installed (returns 503)
- 19 unit tests for connector covering all functionality

### File List
- backend/app/connectors/__init__.py (new)
- backend/app/connectors/sqlserver.py (new)
- backend/app/api/servers.py (modified - added test-connection endpoint and validate param)
- backend/tests/unit/test_sqlserver_connector.py (new)

---

## QA Results
(To be filled by QA agent)
